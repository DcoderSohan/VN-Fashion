import { uploadToCloudinary } from "../config/cloudinary.js";
import { isCloudinaryConfigured } from "../middlewares/uploadMiddleware.js";
import Image from "../models/Image.js";
import path from "path";
import { fileURLToPath } from "url";
import fs from "fs";

// Get current directory
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Helper function to save file locally and return URL
const saveFileLocally = (buffer, mimetype, originalname, folder = 'content') => {
  try {
    // Ensure uploads directory exists
    const uploadsDir = path.join(__dirname, `../uploads/${folder}`);
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }

    // Generate unique filename
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const ext = path.extname(originalname) || '.jpg';
    const name = path.basename(originalname, ext) || 'image';
    const filename = `${name}-${uniqueSuffix}${ext}`;
    const filePath = path.join(uploadsDir, filename);

    // Write buffer to file
    fs.writeFileSync(filePath, buffer);

    // Return relative URL
    const backendDir = path.join(__dirname, '..');
    const uploadsBaseDir = path.join(backendDir, 'uploads');
    const relativePath = path.relative(uploadsBaseDir, filePath);
    
    return {
      url: `/api/uploads/${relativePath.replace(/\\/g, '/')}`,
      localPath: filePath
    };
  } catch (error) {
    console.error('Error saving file locally:', error);
    throw error;
  }
};

// Upload image for content (designer image, gallery, etc.)
export const uploadImage = async (req, res) => {
  try {
    console.log('=== Image Upload Request ===');
    console.log('File present:', !!req.file);
    console.log('File size:', req.file?.size);
    console.log('File mimetype:', req.file?.mimetype);

    if (!req.file) {
      return res.status(400).json({ message: "No image file provided" });
    }

    if (!req.file.buffer) {
      return res.status(400).json({ message: "File buffer is missing" });
    }

    const folder = req.body.folder || 'content';
    const adminId = req.admin?._id || null;
    
    // Generate unique filename
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const ext = path.extname(req.file.originalname) || '.jpg';
    const name = path.basename(req.file.originalname, ext) || 'image';
    const filename = `${name}-${uniqueSuffix}${ext}`;

    let imageUrl = '';
    let publicId = '';
    let localPath = '';
    const useCloudinary = isCloudinaryConfigured();

    if (useCloudinary) {
      try {
        console.log('=== Uploading to Cloudinary ===');
        const cloudinaryFolder = `vn-fashion/${folder}`;
        
        const result = await uploadToCloudinary(
          req.file.buffer,
          req.file.mimetype,
          cloudinaryFolder
        );

        if (!result || !result.secure_url) {
          throw new Error('Cloudinary upload failed - no URL returned');
        }

        imageUrl = result.secure_url;
        publicId = result.public_id;
        console.log('✅ Cloudinary upload successful:', imageUrl);
      } catch (error) {
        console.error('❌ Cloudinary upload error:', error);
        // Fall back to local storage
        console.log('Falling back to local storage...');
        const localResult = saveFileLocally(
          req.file.buffer,
          req.file.mimetype,
          req.file.originalname || 'image.jpg',
          folder
        );
        imageUrl = localResult.url;
        localPath = localResult.localPath;
        console.log('✅ Saved to local storage:', imageUrl);
      }
    } else {
      // Use local storage
      console.log('=== Using Local Storage ===');
      const localResult = saveFileLocally(
        req.file.buffer,
        req.file.mimetype,
        req.file.originalname || 'image.jpg',
        folder
      );
      imageUrl = localResult.url;
      localPath = localResult.localPath;
      console.log('✅ Saved to local storage:', imageUrl);
    }

    // Save image metadata and data to MongoDB
    const imageDoc = await Image.create({
      filename: filename,
      originalName: req.file.originalname || 'image.jpg',
      mimetype: req.file.mimetype,
      size: req.file.size,
      folder: folder,
      data: req.file.buffer, // Store image buffer in MongoDB
      cloudinaryUrl: imageUrl.startsWith('http') ? imageUrl : '',
      publicId: publicId,
      localPath: localPath,
      uploadedBy: adminId,
    });

    console.log('✅ Image saved to MongoDB:', imageDoc._id);

    res.json({
      url: imageUrl,
      imageId: imageDoc._id,
      publicId: publicId,
      message: "Image uploaded and saved to database successfully"
    });
  } catch (error) {
    console.error('❌ Image upload error:', error);
    res.status(500).json({
      message: error?.message || 'Internal server error'
    });
  }
};

// Get images by folder
export const getImagesByFolder = async (req, res) => {
  try {
    const { folder } = req.params;
    const images = await Image.find({ folder: folder })
      .select('-data') // Don't send image data, just metadata
      .sort({ timestamp: -1 })
      .populate('uploadedBy', 'email');

    res.json(images);
  } catch (error) {
    console.error('Error fetching images:', error);
    res.status(500).json({ message: error.message });
  }
};

// Get image by ID (returns image data or URL)
export const getImageById = async (req, res) => {
  try {
    const { id } = req.params;
    const { format } = req.query; // Optional: 'json' or 'image'
    
    const image = await Image.findById(id);

    if (!image) {
      return res.status(404).json({ message: "Image not found" });
    }

    // If format is 'image', serve the image directly
    if (format === 'image') {
      // If Cloudinary URL exists, redirect to it
      if (image.cloudinaryUrl) {
        return res.redirect(image.cloudinaryUrl);
      }

      // If local path exists, serve from local file
      if (image.localPath && fs.existsSync(image.localPath)) {
        return res.sendFile(path.resolve(image.localPath));
      }

      // Serve from MongoDB buffer
      if (image.data) {
        res.set('Content-Type', image.mimetype);
        res.set('Content-Length', image.size);
        return res.send(image.data);
      }

      return res.status(404).json({ message: "Image data not found" });
    }

    // Default: return JSON with URL
    // If Cloudinary URL exists, return that
    if (image.cloudinaryUrl) {
      return res.json({
        _id: image._id,
        filename: image.filename,
        originalName: image.originalName,
        folder: image.folder,
        url: image.cloudinaryUrl,
        size: image.size,
        mimetype: image.mimetype,
        timestamp: image.timestamp
      });
    }

    // If local path exists, return that URL
    if (image.localPath) {
      const backendDir = path.join(__dirname, '..');
      const uploadsBaseDir = path.join(backendDir, 'uploads');
      const relativePath = path.relative(uploadsBaseDir, image.localPath);
      return res.json({
        _id: image._id,
        filename: image.filename,
        originalName: image.originalName,
        folder: image.folder,
        url: `/api/uploads/${relativePath.replace(/\\/g, '/')}`,
        size: image.size,
        mimetype: image.mimetype,
        timestamp: image.timestamp
      });
    }

    // Return image data as base64 if stored in MongoDB
    if (image.data) {
      const base64 = image.data.toString('base64');
      const dataUri = `data:${image.mimetype};base64,${base64}`;
      return res.json({
        _id: image._id,
        filename: image.filename,
        originalName: image.originalName,
        folder: image.folder,
        url: dataUri,
        size: image.size,
        mimetype: image.mimetype,
        timestamp: image.timestamp
      });
    }

    res.status(404).json({ message: "Image data not found" });
  } catch (error) {
    console.error('Error fetching image:', error);
    res.status(500).json({ message: error.message });
  }
};

// Delete image
export const deleteImage = async (req, res) => {
  try {
    const { id } = req.params;
    const image = await Image.findById(id);

    if (!image) {
      return res.status(404).json({ message: "Image not found" });
    }

    // Delete from Cloudinary if exists
    if (image.publicId) {
      try {
        const { deleteFromCloudinary } = await import("../config/cloudinary.js");
        await deleteFromCloudinary(image.publicId);
        console.log('✅ Deleted from Cloudinary:', image.publicId);
      } catch (error) {
        console.error('Error deleting from Cloudinary:', error);
      }
    }

    // Delete local file if exists
    if (image.localPath && fs.existsSync(image.localPath)) {
      try {
        fs.unlinkSync(image.localPath);
        console.log('✅ Deleted local file:', image.localPath);
      } catch (error) {
        console.error('Error deleting local file:', error);
      }
    }

    // Delete from MongoDB
    await Image.findByIdAndDelete(id);

    res.json({ message: "Image deleted successfully" });
  } catch (error) {
    console.error('Error deleting image:', error);
    res.status(500).json({ message: error.message });
  }
};

// Get all folders
export const getFolders = async (req, res) => {
  try {
    const folders = await Image.distinct('folder');
    res.json(folders);
  } catch (error) {
    console.error('Error fetching folders:', error);
    res.status(500).json({ message: error.message });
  }
};
